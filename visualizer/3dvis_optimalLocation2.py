...
# optimal_location = {'x': -4.19, 'y': -33.5286, 'z': 1.85}
optimal_location = {'x': -4.605, 'y': -33.5286, 'z': 1.85}

fov = 90  # 시야각 90도

# 타겟 위치 (결함 표면 위치)
defect_loc = data['defect_node']['location']
...
# 최적 카메라 방향 설정
optimal_camera_direction = np.array([1, 0, 0])

# 시각화를 위한 요소 필터링
elements = data['associated_elements']
fig = go.Figure()
...
for elem in elements:
    ...
    if elem_type == 'Wall' or elem_type in ['Floor', 'Ceiling']:
        ...
        color = 'red' if elem_id == data['defect_node']['id'] else ('lightblue' if elem_type == 'Wall' else 'lightgreen')
...
fig.update_layout(
    scene=dict(
        ...
        aspectmode='cube',  # 모든 축의 비율을 동일하게 설정하여 왜곡 제거
    ),
    title='3D 환경 시각화 - 카메라 시야',
    width=800,
    height=600
)
